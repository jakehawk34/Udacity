...
def find_anagrams(letters, previous_word, shortest):
    ...
    for w in find_words(letters):
        #If the word is longer than the shortest requirement and alphabetically greater than the previous word
        if len(w) >= shortest and w > previous_word:
            remainder = removed(letters, w)
            if remainder:
            # If there are still letters left, recursively call find_anagrams 
            # with the remainder and the previous word
...
def readwordlist(filename):
    ...
    wordset = set(open(filename).read().upper().replace('\\', '').split())
    prefixset = set(p for word in wordset for p in prefixes(word))
    return wordset, prefixset

WORDS, PREFIXES = readwordlist('words4k.rtf')

WORDS = sorted(WORDS)[:-4] #3902 words

PREFIXES = sorted(PREFIXES)[:6599]

last = PREFIXES.index('ZYMURG') #6598 prefixes
