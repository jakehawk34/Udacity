...
WORDS = sorted(WORDS)[:-5] #3901 words

PREFIXES = sorted(PREFIXES)[:6599]

last = PREFIXES.index('ZYMURG') #6599 prefixes
...
def show_best(hand, board):
    "Show the best play for the current hand and board. Or None if there are no possible plays."
    print('Current board:')
    show(board)
    play = best_play(hand, board)
    if play:
        print(f"The best word is '{play[-1]}', which has a score of {play[0]}.")
        show(make_play(play, board))
    else:
        print('Sorry, no legal plays.')

show(a_board())
print(best_play('ABCDEKH', a_board()))
