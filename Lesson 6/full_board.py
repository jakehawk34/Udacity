...
def horizontal_plays(hand, board):
    ...
    results = set()
    for (j, row) in enumerate(board[1:-1], 1): #Checks each column in each row
        set_anchors(row, j, board) #Sets the anchor squares for each row
        for (i, word) in row_plays(hand, row): #For each (start, word) pair in possible row_plays
            results.add(((i, j), word)) #Add a ((row, column), word) tuple to results
    return results
...
def all_plays(hand, board):
    ...
    vplays = horizontal_plays(hand, transpose(board)) # set of ((j, i), word) E.g. (1, 0) on board = (0, 1) on transpose(board)
    for ((i, j), word) in hplays: 
        results.add(((i, j), ACROSS, word)) #Add to results with (i, j)
    for ((j, i), word) in vplays:
        results.add(((i, j), DOWN, word)) #Add to results with (i, j) by flipping the transposed board coordinates
    return results
