...
def bowling(balls):
    ...
    score = 0
    frame = 0
    if len(balls) > 21: # edge case for no spares/strikes for first 9 frames, three rolls for 10th frame
        return 'Too many rolls'
    elif len(balls) < 11: # edge case for nine strikes for first 9 frames, two rolls for 10th frame
        return 'Not enough rolls'
    else:
        while frame < 10: 
            frame += 1 # Increment frame by 1
            first_roll = balls[0]
            second_roll = balls[1]
            if is_strike(first_roll):
                score += sum(balls[0:3]) # Strike : score increased by 10 and the next two rolls; frame used one ball
                balls = balls[1:]
            elif is_spare(first_roll, second_roll): # Spare: score increased by 10 and the next roll; frame used two balls
                score += sum(balls[0:3])
                balls = balls[2:]
            else: # Score increased by the two most recent rolls; frame used two balls
                score += sum(balls[0:2])
                balls = balls[2:]
        return score


def is_strike(roll): #Returns true if the first roll of a frame is a 10
    return roll == 10

def is_spare(first_roll, second_roll): #Returns true if the sum of two rolls in a frame is 10
