...
def Pwin3(me, you, pending):
    if me + pending >= goal: #100% chance of winning if player's score + pending is >= to the goal
        return 1
    elif you >= goal: #0% chance of winning if opponent's score is >= to the goal
        return 0
    else:
        '''Proll represents the probability of the player winning from this state
        Equivalent to 1 minus the p'''
...
print(timedcall(Pwin2((0, 0, 0, 0)))) #Time it takes to run the function
print(Pwin2((0, 0, 0, 0))) #Probability of winning if you go first (should be a little above 50%)
print(len(Pwin3.cache)) #Number of items in the probability memo cache
#print(test())
